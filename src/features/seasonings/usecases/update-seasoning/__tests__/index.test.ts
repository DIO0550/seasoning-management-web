...
beforeEach(() => {
  ...
  useCase = new UpdateSeasoningUseCase(mockSeasoningRepository);
});

test("正常系: 名前のみを更新した場合、更新後のデータを返す", async () => {
  const originalEntity = createSeasoningEntity();
  const updatedEntity = createSeasoningEntity({ name: "濃口醤油" });
...
test("正常系: imageIdをnullに更新した場合、画像が削除される", async () => {
  const originalEntity = createSeasoningEntity();
  const updatedEntity = createSeasoningEntity({ imageId: null });
...
test("正常系: imageIdを新しい値に更新した場合、画像が変更される", async () => {
  const originalEntity = createSeasoningEntity();
  const updatedEntity = createSeasoningEntity({ imageId: 20 });
