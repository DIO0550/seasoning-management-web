...
describe("useSeasoningSubmit", () => {
  ...
  test("初期状態が正しく設定されること", () => {
    ...
    expect(result.current.errors.general).toBe(SUBMIT_ERROR_STATES.NONE);
  ...
  describe("リファクタリング後の単一責任テスト", () => {
    test("バリデーション処理が独立して動作すること", async () => {
      const { validateImage } = await import("@/utils/image-validation/image-validation");
  ...
  describe("リファクタリング後の責務分離テスト", () => {
    ...
    test("setImageErrorで画像エラーのみが更新されること", () => {
      ...
      expect(result.current.errors.general).toBe(SUBMIT_ERROR_STATES.NONE); // generalエラーは変更されない
    });
  });
