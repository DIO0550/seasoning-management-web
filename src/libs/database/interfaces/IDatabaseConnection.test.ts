...
import type { IDatabaseConnection, QueryResult, ConnectionConfig } from './IDatabaseConnection';
...
describe('IDatabaseConnection', () => {
  ...
  test('未接続状態でクエリを実行するとエラーが発生する', async () => {
    ...
    await expect(connection.query('SELECT 1')).rejects.toThrow('Database is not connected');
  });
  ...
  test('未接続状態でトランザクションを開始するとエラーが発生する', async () => {
    ...
    await expect(connection.beginTransaction()).rejects.toThrow('Database is not connected');
  });
